defmodule Azar.Servicios.GestorClientes do
      ...
      sorteo ->
        ...
        Enum.each(1..total, fn num ->
          if num in numeros_completos do
            ...
            fracs_libres = Enum.reject(1..fracciones |> Enum.to_list(), fn f -> f in fracs_tomadas end)
  ...
  def comprar_billete(cliente_id, sorteo_id, numero, tipo, fraccion_numero \\ nil) do
    ...
    with {:ok, sorteo} <- encontrar_sorteo_activo(sorteos, sorteo_id),
         ...
         :ok           <- validar_disponibilidad(billetes, sorteo_id, numero, tipo, fraccion_numero, sorteo["fracciones"]),
  ...
  def historial_compras(cliente_id) do
    ...
    if mis_compras == [] do
      ...
      Enum.each(mis_compras, fn b ->
        ...
        IO.puts("  #{b["fecha_compra"]} | #{nombre_sorteo} | ##{b["numero"]} (#{b["tipo"]}) | $#{b["valor_pagado"]}#{devuelto}")
      end)
  ...
  def devolver_compra(billete_id, cliente_id) do
    ...
    case Enum.find(billetes, fn b -> b["id"] == billete_id end) do
      ...
      billete when billete["cliente_id"] != cliente_id ->
        {:error, "Este billete no pertenece a tu cuenta"}

      billete when billete["devuelto"] ->
      ...
      billete ->
        ...
        if sorteo["realizado"] do
          ...
          case JsonHelper.escribir_archivo(@archivo_billetes, billetes_actualizados) do
            :ok ->
              IO.puts("✅ Billete ##{billete["numero"]} devuelto. Se reembolsarán $#{billete["valor_pagado"]}")
  ...
  defp encontrar_sorteo_activo(sorteos, sorteo_id) do
    case Enum.find(sorteos, fn s -> s["id"] == sorteo_id end) do
      ...
      s when s["realizado"] -> {:error, "El sorteo ya fue realizado"}
      sorteo -> {:ok, sorteo}
    end
  end
  ...
  defp validar_disponibilidad(billetes, sorteo_id, numero, tipo, fraccion_numero, total_fracciones) do
    activos = Enum.filter(billetes, fn b ->
